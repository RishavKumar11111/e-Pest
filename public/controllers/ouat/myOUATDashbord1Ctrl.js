app.controller('myOUATDashbord1Ctrl', function ($scope, $http, $filter) {
    ...
    $scope.getDashboardDetails = function () {
        $http.get('http://localhost:3000/ouat/getDashboardDetails?season=' + $scope.cbSeason + '&financialYear=' + $scope.ddlFY).then(function success(response) {
    ...
    $scope.getPestGraphData = function () {
        if ((($scope.ddlMonth == undefined || $scope.ddlMonth == null) && ($scope.ddlFY == undefined && $scope.ddlFY == null)) || ($scope.ddlMonth != undefined && $scope.ddlMonth != null && $scope.ddlFY != undefined && $scope.ddlFY != null) || (($scope.ddlMonth == undefined || $scope.ddlMonth == null) && $scope.ddlFY != undefined && $scope.ddlFY != null)) {
            ...
            if ($scope.pests.length > 0) {
                ...
                $http.post('http://localhost:3000/ouat/getPestGraphData', { data: { pestData: $scope.pests, month: month, season: season, financialYear: financialYear } }, { credentials: 'same-origin', headers: { 'CSRF-Token': token } }).then(function success(response) {
    ...
    $scope.getGD = function () {
        ...
        var affectedPestArea = [];
        var categoryCode = [];
        $http.get('http://localhost:3000/ouat/getGraphforCrop?season=' + $scope.cbSeason + '&financialYear=' + $scope.ddlFY).then(function success(response) {
    ...
    document.getElementById('myChart9').onclick = function (evt) {
        ...
        if (activePoints[0]) {
            ...
            var bagColor = [];
            httpGetAsync('http://localhost:3000/ouat/getCropDetailsCategory?cropCode=' + cropCode + '&season=' + $scope.cbSeason + '&financialYear=' + $scope.ddlFY, function (res) {
