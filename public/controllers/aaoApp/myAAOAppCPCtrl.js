app.controller('myAAOAppCPCtrl', function($scope, $http, $window) {
            ...
            readAllData('user-authentication').then(function success(resun) {
                ...
                if (sha256($scope.txtOldPassword) !== obj.NewPassword) {
                    if (obj.NewPassword === obj.ConfirmPassword) {
                        $http.post('http://localhost:3000/aaoApp/changePassword', { data: obj }, { credentials: 'same-origin', headers: { 'CSRF-Token': token } }).then(function success(response) {
