app.controller('myCDAOComplianceCtrl', function($scope, $http) {
        ...
        if ((month >= 6 && month <= 10 && $scope.rbs == 'K') || ((month < 6 || month > 10) && $scope.rbs == 'R')) {
            if (day == 0) {
                ...
                if ($scope.dateOfPDE != null && $scope.dateOfPDE != undefined && $scope.rbs != null && $scope.rbs != undefined && $scope.ddlFY != null && $scope.ddlFY != undefined) {
                    $http.get('http://localhost:3000/cdao/getComplianceReport?dateOfEntry=' + document.getElementById('pdeDate').value + '&season=' + $scope.rbs + '&financialYear=' + $scope.ddlFY).then(function success(response) {
    ...
    $scope.getTargetedGP = function (blockCode) {
            $http.get('http://localhost:3000/cdao/getTargetedGP?blockCode=' + blockCode + '&season=' + $scope.rbs).then(function success(response) {
    ...
    $scope.getSurveyGP = function (blockCode) {
        $http.get('http://localhost:3000/cdao/getSurveyGP?dateOfEntry=' + document.getElementById('pdeDate').value + '&blockCode=' + blockCode + '&season=' + $scope.rbs).then(function success(response) {
