app.controller('myCDAOVDReportCtrl', function($scope, $http) {
    ...
    $scope.getVAWDetailsReport = function (blockCode) {
        $http.get('http://localhost:3000/cdao/getVAWDetailsReport?blockCode=' + blockCode).then(function success(response) {
    ...
    $scope.getVAWDPTargets = function (blockCode) {
        $http.get('http://localhost:3000/cdao/getVAWDPTargets?blockCode=' + blockCode).then(function success(response) {
            $scope.vawGPTargets = response.data;
        }, function error(response) {
            console.log(response.status);
        }).catch(function err(error) {
            console.log('An error occurred...', error);
