app.controller('myCDAOVAWIRCtrl', function($scope, $http) {
    ...
    $scope.getVAWInspectionDetails = function() {
        if ($scope.dateOfPDE != null && $scope.dateOfPDE != undefined && $scope.rbs != null && $scope.rbs != undefined && $scope.ddlFY != null && $scope.ddlFY != undefined) {
            ...
            $http.get('http://localhost:3000/cdao/getVAWInspectionDetails?dateOfEntry=' + document.getElementById('pdeDate').value + '&season=' + $scope.rbs + '&financialYear=' + $scope.ddlFY + '&blockCode=' + blkcd).then(function success(response) {
    ...
    $scope.getCD = function (referenceNo) {
        $http.get('http://localhost:3000/cdao/getCD?referenceNo=' + referenceNo).then(function success(response) {
    ...
    $scope.getPLD = function (referenceNo) {
        $http.get('http://localhost:3000/cdao/getPLD?referenceNo=' + referenceNo).then(function success(response) {
    ...
    $scope.getPD = function (referenceNo) {
        ...
        $http.get('http://localhost:3000/cdao/getPD?referenceNo=' + referenceNo).then(function success(response) {
