app.controller('myADODashboardCtrl', function ($scope, $http) {
    ...
    $scope.getDashboardDetails = function () {
        $http.get('http://localhost:3000/ado/getDashboardDetails?season=' + $scope.cbSeason + '&financialYear=' + $scope.ddlFY).then(function success(response) {
            $scope.getADODD = response.data;
            document.getElementById('myChart6').style.pointerEvents = 'none';
            var taa = $scope.getADODD[5][0].TotalAffectedArea;
            var tat = $scope.getADODD[5][0].TotalTreatedArea;
    ...
    $scope.getPestDiseases = function () {
        if ($scope.ddlCropCategory != undefined) {
            $http.get('http://localhost:3000/ado/getPestDiseases?cropCode=' + $scope.ddlCrop).then(function success(response) {
    ...
    $scope.getPestGraphData = function () {
        if ((($scope.ddlMonth == undefined || $scope.ddlMonth == null) && ($scope.ddlFY == undefined && $scope.ddlFY == null)) || ($scope.ddlMonth != undefined && $scope.ddlMonth != null && $scope.ddlFY != undefined && $scope.ddlFY != null) || (($scope.ddlMonth == undefined || $scope.ddlMonth == null) && $scope.ddlFY != undefined && $scope.ddlFY != null)) {
            ...
            if ($scope.pests.length > 0) {
                ...
                $http.post('http://localhost:3000/ado/getPestGraphData', { data: { pestData: $scope.pests, month: month, season: season, financialYear: financialYear } }, { credentials: 'same-origin', headers: { 'CSRF-Token': token } }).then(function success(response) {
    ...
    $scope.getGD = function () {
        ...
        var affectedPestArea = [];
        var categoryCode = [];
        $http.get('http://localhost:3000/ado/getGraphforCrop?season=' + $scope.cbSeason + '&financialYear=' + $scope.ddlFY).then(function success(response) {
    ...
    document.getElementById('myChart9').onclick = function (evt) {
        ...
        if (activePoints[0]) {
            ...
            var bagColor = [];
            httpGetAsync('http://localhost:3000/ado/getCropDetailsCategory?cropCode=' + cropCode + '&season=' + $scope.cbSeason + '&financialYear=' + $scope.ddlFY, function (res) {
