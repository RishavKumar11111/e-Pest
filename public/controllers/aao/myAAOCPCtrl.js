app.controller('myAAOCPCtrl', function ($scope, $http) {
        ...
        if (isValid) {
            ...
            if (sha256($scope.txtOldPassword) !== obj.NewPassword) {
                if (obj.NewPassword === obj.ConfirmPassword) {
                    $http.post('http://localhost:3000/aao/changePassword', { data: obj }, { credentials: 'same-origin', headers: { 'CSRF-Token': token } }).then(function success(response) {
